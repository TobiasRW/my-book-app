...
export default async function MoreBooks({ primaryAuthor, currentBookId, currentTitle }) {
    ...
    return (
        <div className="">
            <h2 className='font-bold text-lg truncate mt-4'>More by {primaryAuthor}</h2>
            <div className="flex gap-5 overflow-x-scroll no-scrollbar">
                {moreBooksError && (
                    <div className="bg-darkgray rounded-xl p-4 text-offwhite text-center w-full my-4">
                        {moreBooksError}
                    </div>
                )}
                {moreBooks.length > 0 && (
                    <MoreBookList books={moreBooks} />
                )}
            </div>
        </div>
    );
